...
import duckdb
from reladiff import connect, connect_to_table, diff_tables

from tablediff.models import DiffResult, SchemaDiffResult, TableMeta
...
def table_diff(db_path_a, db_path_b, table_a_name, table_b_name, primary_key, where=None) -> DiffResult:
    ...
    diffing = diff_tables(table_a, table_b, key_columns=(primary_key,), extra_columns=tuple(common_cols), where=where)
...
def schema_diff(db_path_a, db_path_b, table_a_name, table_b_name) -> SchemaDiffResult:
    ...
    for col in sorted(all_columns):
        ...
        columns[col] = {"table_a": type_a, "table_b": type_b}

    return SchemaDiffResult(table_a=table_a_name, table_b=table_b_name, columns=columns)
